# Search-R1 lite (standard tool calling)

This is a minimal reproduction of [Search-R1](https://github.com/PeterGriffinJin/Search-R1) and an example of using multi-turn conversation and standard tool-calling in slime.

## Tool-Calling Format

This variant uses standard tool-calling blocks instead of `<search>`/`<answer>` tags.

### Tool Calls (assistant output)

Search request:

```text
<tool_call>{"name":"search","arguments":{"query":"weather in Hong Kong"}}</tool_call>
```
...
### Tool Responses (environment feedback)

Search results are injected back into the running transcript as:

```text
<tool_response>{"name":"search","status":"success","content":"Doc 1..."}</tool_response>
```

If the tool fails:

```text
<tool_response>{"name":"search","status":"error","error":"..."}</tool_response>
```
...
```bash
cd slime/
bash examples/search-r1_standard_tool_calling/run_qwen2.5_3B.sh
```
...
python /root/slime/examples/search-r1_standard_tool_calling/local_dense_retriever/download.py --save_path $save_path
...
python /root/slime/examples/search-r1_standard_tool_calling/local_dense_retriever/retrieval_server.py \
...
bash /root/slime/examples/search-r1_standard_tool_calling/run_qwen2.5_3B.sh
```
