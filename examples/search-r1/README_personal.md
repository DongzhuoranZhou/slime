...
```bash
cd /workspace/src/clean_code_for_rl/
...
WORK_DIR=/workspace/src/clean_code_for_rl/Search-R1
...
```bash
# hf checkpoint
hf download Qwen/Qwen2.5-3B --local-dir /workspace/.cache/huggingface/hub/Qwen2.5-3B

# mcore checkpoint
cd /workspace/src/clean_code_for_rl/slime
source scripts/models/qwen2.5-3B.sh
PYTHONPATH=/workspace/src/clean_code_for_rl/Megatron-LM python tools/convert_hf_to_torch_dist.py \
    ${MODEL_ARGS[@]} \
    --hf-checkpoint /workspace/.cache/huggingface/hub/Qwen2.5-3B \
    --save /workspace/.cache/huggingface/hub/Qwen2.5-3B_torch_dist
```
...
python /workspace/src/clean_code_for_rl/slime/examples/search-r1/local_dense_retriever/download.py --save_path $save_path
...
# Set cache directory to avoid filling /workspace
