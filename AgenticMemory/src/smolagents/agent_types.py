...
class AgentImageList(AgentType):
    """
    List of Images returned by the agent.
    """
    def __init__(self, value):
        AgentType.__init__(self, value)
        ...
        # if all element in value are AgentImage, extract their raw, path, tensor
        if all(isinstance(v, AgentImage) for v in value):
            self._raw_list = [v._raw for v in value]
            self._path_list = [v._path for v in value]
            self._tensor_list = [v._tensor for v in value]
        # if all element in value are PIL.Image.Image, set raw_list
        elif all(isinstance(v, PIL.Image.Image) for v in value):
            self._raw_list = value
        # if all element in value are bytes, convert to PIL.Image.Image and set raw_list
        elif all(isinstance(v, bytes) for v in value):
            self._raw_list = [PIL.Image.open(BytesIO(v)) for v in value]
        # if all element in value are str or pathlib.Path, set path_list
        elif all(isinstance(v, (str, pathlib.Path)) for v in value):
    ...
    def to_raw(self):
        """
        Returns the "raw" version of that object. In the case of an AgentImageList, it is a list of PIL.Image.Image.
        """
        # if self._raw_list is not an empty list return it
        if self._raw_list:
        ...
        if self._path_list:
            ...
            for path in self._path_list:
                self._raw_list.append(PIL.Image.open(path))
            return self._raw_list
        
    def to_string(self):
        """
        Returns the stringified version of that object. In the case of an AgentImageList, it is a list of paths to the serialized
        version of the images.
        """
        if self._path_list:
            return str(self._path_list)

        if self._raw_list:
            return str(self._raw_list)
...
def handle_agent_output_types(output: Any, output_type: str | None = None) -> Any:
    ...
    if isinstance(output, list) and all(isinstance(elem, PIL.Image.Image) for elem in output):
        return AgentImageList(output)
    try:
        import torch
...
__all__ = ["AgentType", "AgentImage", "AgentImageList", "AgentText", "AgentAudio"]
