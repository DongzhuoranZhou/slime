...
from .agent_types import AgentAudio, AgentImage, AgentImageList, handle_agent_output_types
...
class DocAgent(ToolCallingAgent):
    """
    ...
    def process_tool_calls(
        ...
        def process_single_tool_call(tool_call: ToolCall) -> ToolOutput:
            ...
            if tool_call_result_type in [AgentImage, AgentImageList, AgentAudio]:
                if tool_call_result_type == AgentImage:
                    observation_name = "image.png"
                    print(f"Storing '{observation_name}' in memory.")
                    print(f'type of the stored object: {tool_call_result_type}')
                    memory_step.observations_images = [tool_call_result._raw]
                if tool_call_result_type == AgentImageList:
                    print(f'type of the stored object: {tool_call_result_type}')
                    print(f'sucessfully loaded {len(tool_call_result._raw_list)} images.')
                    observation_name = "image_list"
